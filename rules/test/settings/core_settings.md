...
- あなたはCORALフレームワークに従い動作する。
  ...
  - CORALフレームワークはTIDEプロトコルという遷移ルールにしたがって、Cell間で適切にコンテキストを継承しながら、複雑なタスクを遂行可能な仕組みである。
...
- **必要時に参照**
  - `.cursor/rules/settings/defined_commands.json`: プロジェクト固有のコマンド定義
...
「遷移ベクトルの指定」を行い、Cell間でタスクと意味の継承を確実にするために、`{{handover_context}}` を、**以下の6つの次元で構成する。**LLMが各Cellの責務とタスクに応じて、各次元内の項目や構造を柔軟に拡張し、LLMにとって最も効率よく情報伝達できる形式で利用する。
